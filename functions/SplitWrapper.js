...
export default function SplitStorageWrapper (storeId)  {
  if(storeId) {
  ...
  return {
    ...
    async getAndSet(key, value) {
      return (split.get(key, { type: 'strong' })
      .then(val => split.set(key, value).then(() => val))).json()
    },
    async del(key) {
      return (await split.delete(key)).ok
    },
    async getKeysByPrefix(prefix) {
      return (await split.list().filter(x => x.key.startsWith(prefix)).map(x => x.key)).json();
    },
    async getMany(keys) {
      return (await keys.map(async key => await split.get(key, {type: 'strong'}))).json();
    },
    ...
    async itemContains(key, item) {
      return (await split.get(key, {type: 'strong'}).then(x => x.has(item))).json();
    },    
